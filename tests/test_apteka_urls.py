...
def test_order_base_url_defaults_to_api_domain(monkeypatch, tmp_path) -> None:
    monkeypatch.delenv("APTEKA_ORDER_BASE_URL", raising=False)
    monkeypatch.setattr(apteka_urls, "ENV_FILE_PATH", tmp_path / "missing.env")

    assert apteka_urls.get_apteka_order_base_url() == "https://api.apteka.md"
    assert apteka_urls.build_order_front_url("/order/confirm-order-by-using-mobile") == (
        "https://api.apteka.md/api/v1/front/order/confirm-order-by-using-mobile"
    )
